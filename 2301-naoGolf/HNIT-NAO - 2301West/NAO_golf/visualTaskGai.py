...
class StickDetect(VisualBasis):
    def __init__(self, robot_ip, port=9559, camera_id=vd.kTopCamera):
        ...
        self.contourRange = [120, 650]  # 实测最大450左右
        self.minHsv = np.array([0, 100, 160])  # [20, 100, 40]
        self.maxHsv = np.array([44, 255, 255])  # [44, 255, 255]
...
#         self.markContour = []
#         self.markAngle = 0.0
#         # 移除Flask URL，添加NAOqi连接
#         # try:
#         self.session = naoqi.ALProxy("ALProxy", robot_ip, port)
#         # except:
#         #     # 如果ALProxy不可用，使用其他方式创建session
#         #     self.session = None
...
#
#         data = self._get_mark_data_via_naoqi()  # 新增的NAOqi检测方法
#
...
#             self.markAngle = 0.0
#
#     def _get_mark_data_via_naoqi(self):
#         """使用NAOqi SDK检测Mark并返回与Flask服务相同的格式"""
#         try:
#             # 1. 连接到NAOqi的Mark检测模块
#             land_mark_proxy = self.session.service("ALLandMarkDetection")
#             memory_proxy = self.session.service("ALMemory")
#
#             # 2. 订阅Mark检测服务
#             subscription_period = 500  # 检测周期500ms
#             land_mark_proxy.subscribe("LandmarkDetector", subscription_period, 0.0)
#
#             # 3. 获取Mark检测数据
#             mark_data = memory_proxy.getData("LandmarkDetected")
#
#             # 4. 取消订阅（可选，根据需求决定）
#             land_mark_proxy.unsubscribe("LandmarkDetector")
#
#             # 5. 解析数据并转换成与Flask服务相同的格式
#             if mark_data and isinstance(mark_data, list) and len(mark_data) >= 2:
#                 marks_list = mark_data[1]  # Mark信息列表
#
#                 if marks_list and len(marks_list) > 0:
#                     # 取第一个检测到的Mark
#                     first_mark = marks_list[0]
#
#                     # 6. 将NAOqi的3D坐标转换为2D图像像素坐标
#                     return self._convert_naoqi_to_flask_format(first_mark)
#
#             # 没有检测到Mark
#             return None
#
#         except Exception as e:
#             print("NAOqi Mark检测错误: {e}")
#             return None
#
#     def _convert_naoqi_to_flask_format(self, naoqi_mark_data):
#         """将NAOqi的Mark数据转换为Flask服务的JSON格式"""
#         try:
#             # naoqi_mark_data结构: [MarkID, 形状信息, 位置信息, 像素信息]
#             # 位置信息: [x, y, z, wx, wy, wz] (米和弧度)
#             position_info = naoqi_mark_data[2]
#
#             # 简化的3D到2D投影转换
#             x_3d, y_3d, z_3d = position_info[0], position_info[1], position_info[2]
#
#             # 相机参数（使用NAO摄像头的典型值）
#             focal_length = 525  # 像素
#             center_x, center_y = 320, 240  # 图像中心(640x480)
#
#             if z_3d > 0:
#                 # 透视投影计算
#                 x_2d = int((x_3d / z_3d) * focal_length + center_x)
#                 y_2d = int((y_3d / z_3d) * focal_length + center_y)
#
#                 # 估算边界框大小（根据距离调整）
#                 base_size = 50  # 基础大小（像素）
#                 distance_factor = 1.0 / max(z_3d, 0.5)  # 距离越远框越小
#                 box_size = int(base_size * distance_factor)
#
#                 # 计算边界框坐标（保持与YOLO相同的格式）
#                 x1 = max(0, x_2d - box_size // 2)
#                 y1 = max(0, y_2d - box_size // 2)
#                 x2 = min(639, x_2d + box_size // 2)
#                 y2 = min(479, y_2d + box_size // 2)
#
#                 # 返回与Flask服务完全相同的格式
#                 return {
#                     'pt1': [x1, y1],  # 左上角
...
class LandMarkDetect(VisualBasis):
    def __init__(self, robot_ip, port=9559, camera_id=vd.kTopCamera):
        ...
        # 以下新增黄杆检测相关参数
        self.crop = 1  # 图片裁剪比例
        self.contourRange = [120, 650]  # 轮廓周长范围
        self.minHsv = np.array([25, 100, 32])  # HSV最小值
        self.maxHsv = np.array([44, 255, 255])  # HSV最大值
        self.cameraProxy.setActiveCamera(self.cameraID)
